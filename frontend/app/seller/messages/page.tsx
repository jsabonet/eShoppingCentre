        ...
        <div className="flex h-[calc(100vh-180px)] bg-card border border-border rounded-xl overflow-hidden">
          ...
          <div className={`${activeConv ? 'hidden md:flex' : 'flex'} md:flex flex-col w-full md:w-80 border-r border-border shrink-0`}>
            <div className="p-4 border-b border-border">
              ...
              <p className="text-xs text-muted-foreground mt-1">Conversas com os seus clientes</p>
            </div>
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                <div className="text-center py-12 px-4">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Clientes podem iniciar conversas a partir da sua loja.</p>
                </div>
              ) : (
                conversations.map(conv => (
                  <button key={conv.id} onClick={() => setActiveConvId(conv.id)}
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium truncate">{conv.buyer_name}</span>
                    ...
                    {conv.unread_count > 0 && (
                      <span className="inline-block mt-1 bg-accent text-accent-foreground text-[10px] px-1.5 py-0.5 rounded-full">
              ...
              <>
                <div className="px-4 py-3 border-b border-border flex items-center gap-3">
                  ...
                  <div className="w-8 h-8 rounded-full bg-accent/10 flex items-center justify-center text-sm font-bold text-accent shrink-0">
                    {activeConv.buyer_name?.[0] || '?'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold truncate">{activeConv.buyer_name}</p>
